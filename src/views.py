...
def reading_xlsx(path:str, date:str) -> pd.DataFrame:
...
def each_card(path: str, date:str) -> list[dict]:
    ...
    df = reading_xlsx(path=path, date=date)
    if len(df) == 0:
        return []
    else:
        ...
        df = df_expanse.merge(df_operation, how='outer', on=['Номер карты', 'Сумма операции'])
...
def top_transactions(path, date) -> list[dict]:
    ...
    if len(df) == 0:
        ...
        df = pd.concat([df_expanse, df_operation])
        df['Сумма операции'] = abs(df['Сумма операции'])
        df['Дата операции'] = df['Дата операции'].dt.strftime('%d.%m.%Y')
        df = df.drop_duplicates().sort_values('Сумма операции', ascending=False)[0:4]
...
def greetings(date: str) -> str:
    ...
    hour = int(pd.to_datetime(date).strftime('%H'))
    if 6<=hour<=12:
        return 'Доброе утро'
    elif 13<=hour<=17:
        return 'Добрый день'
